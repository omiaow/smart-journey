...
class AutocompleteTo extends React.Component {
  ...
  tags(){
    ...
    var returnValue = (<div className="collect-tag">{tag_list}</div>);
    return returnValue;
  }
    ...
    if(this.state.value.length > 0){
      ...
      for(var i=0; i<CityList.length; i++){
        if(check_substring(this.state.value, CityList[i].loc) && j < this.state.autocompleteLength){
          ...
          list.push(<div className="autocomplete-tag" key={j} onClick={this.choose.bind(null, city.split(',')[0], CityList[i].id)}> {city} </div>);
          j++;
    ...
    return(
      <>
        <div className="to" onClick={() => this.props.changeInput("toInput")}>
          <div data-testid="icon" className="icon"></div>
          <span data-testid="placeholder">{(this.state.placeholder.length > 0) ? (`To ${this.state.placeholder}`) : ("To where? (city, country)")}</span>
          {this.showInputs()}
        </div>
